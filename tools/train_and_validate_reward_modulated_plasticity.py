...
class RewardModulatedMillionCellBrain(nn.Module):
    """
    三因子强化突触塑性 (Reward-Modulated Plasticity) + 宏观生境门控百万细胞生命体
    """
    def __init__(self, num_assets=18, num_cells=1000000, num_synapses=2000000, device='cuda', seed=42):
        ...
        self.register_buffer('last_eligibility', torch.zeros((actual_synapses,), dtype=torch.float16, device=device))

    def reset(self):
        self.state.zero_()
        self.last_eligibility.zero_()

    @torch.no_grad()
    def forward_pass(self, batch_features):
        ...
        self.last_eligibility.copy_((src_signals * dst_signals).mean(dim=0).to(dtype=torch.float16))
    ...
    @torch.no_grad()
    def apply_reward_plasticity(self, reward_signal, lr=0.0005):
        if lr <= 0 or abs(reward_signal) < 1e-5:
            return
        r = max(-2.0, min(2.0, reward_signal * 50.0))
        delta_w = self.last_eligibility * (r * lr)
...
def evaluate_reward_modulated_brain(brain, data, timeline, d_start, d_end, fee_mult=1.0, slip_mult=1.0, top_k=3, enable_reward_plasticity=True, lr=0.0005, enable_regime_gate=True):
    ...
    for t, cur_date in enumerate(sub_timeline):
        ...
        # 3. 三因子多巴胺可塑性更新
        if enable_reward_plasticity and not is_last_day:
            brain.apply_reward_plasticity(d_ret, lr=lr)
        ...
        if not is_last_day and not trading_halted:
            ...
            scores, immunes = brain.forward_pass(feat_tensor)
            
            # 生境离散度自适应缩放 (0.3x ~ 1.2x)
            dispersion = np.std(scores) if len(scores) > 0 else 0.0
            mean_abs = np.mean(np.abs(scores)) + 1e-6
            relative_disp = dispersion / mean_abs
            
            regime_scaler = 1.0
            if enable_regime_gate:
                if relative_disp < 0.6:
                    regime_scaler = 0.4  # 相对离散度低 -> 压缩仓位规避摩擦
                elif relative_disp > 1.2:
                    regime_scaler = 1.2  # 相对离散度高 -> 扩大阿尔法
            ...
            for i, sym in enumerate(asset_symbols):
                ...
                contracts = min(6, int((total_equity * 0.04 * regime_scaler) / margin_per_contract)) if margin_per_contract > 0 else 0
...
def main():
    ...
    print(f"\n==========================================================================================================", flush=True)
    print(f" 🚀 终极跃迁：三因子强化多巴胺突触塑性 (Reward-Modulated) + 生境离散度门控 10.7 年盲测 🚀", flush=True)
    print(f"==========================================================================================================", flush=True)
    print(f"• 硬件加速: {torch.cuda.get_device_name(0) if device=='cuda' else 'CPU'} (CUDA Float16)", flush=True)
    print(f"• 学习机制: 强化赫布三因子 (仅在真实盈利时强化突触，亏损时惩罚衰减，彻底灭绝假突破追噪)", flush=True)
    print(f"• 生境门控: 截面相对离散度门控 (混沌期自动压缩仓位，高分化期捕获阿尔法)", flush=True)
    ...
    print(f"【消融大比武】样本外 (2016 ~ 2026, 10.7 年) 各类机制演化对比:", flush=True)
    print(f"{'-'*112}", flush=True)
    print(f"{'模型机制 / 演化形态':<34}{'期末清盘现金':<18}{'总回报率':<14}{'CAGR':<14}{'最大回撤':<14}{'胜率':<12}{'Calmar':<10}", flush=True)
    print(f"{'-'*112}", flush=True)

    # 1. 静态冻结大脑
    brain_static = RewardModulatedMillionCellBrain(num_assets=num_assets, device=device, seed=42)
    res_static = evaluate_reward_modulated_brain(brain_static, data, timeline, split_date, timeline[-1], enable_reward_plasticity=False, enable_regime_gate=False)
    print(f"{'1. 静态冻结大脑 (无自适应)':<32}{res_static['final_cash']:,.2f} 元{'':<4}{res_static['roi']:+.2f}%{'':<6}{res_static['cagr']:+.2f}%{'':<6}{res_static['max_dd']:.2f}%{'':<6}{res_static['win_rate']:.1f}%{'':<6}{res_static['calmar']:.2f}", flush=True)

    # 2. 仅加生境离散度门控
    brain_gated = RewardModulatedMillionCellBrain(num_assets=num_assets, device=device, seed=42)
    res_gated = evaluate_reward_modulated_brain(brain_gated, data, timeline, split_date, timeline[-1], enable_reward_plasticity=False, enable_regime_gate=True)
    print(f"{'2. 生境离散度门控 (防震荡磨损)':<30}{res_gated['final_cash']:,.2f} 元{'':<4}{res_gated['roi']:+.2f}%{'':<6}{res_gated['cagr']:+.2f}%{'':<6}{res_gated['max_dd']:.2f}%{'':<6}{res_gated['win_rate']:.1f}%{'':<6}{res_gated['calmar']:.2f}", flush=True)

    # 3. 三因子多巴胺强化塑性 + 生境门控 (以变化对抗变化终极形态)
    brain_reward = RewardModulatedMillionCellBrain(num_assets=num_assets, device=device, seed=42)
    res_reward = evaluate_reward_modulated_brain(brain_reward, data, timeline, split_date, timeline[-1], enable_reward_plasticity=True, lr=0.0003, enable_regime_gate=True)
    print(f"{'3. 🔥 三因子多巴胺塑性 + 生境门控':<28}{res_reward['final_cash']:,.2f} 元{'':<4}{res_reward['roi']:+.2f}%{'':<6}{res_reward['cagr']:+.2f}%{'':<6}{res_reward['max_dd']:.2f}%{'':<6}{res_reward['win_rate']:.1f}%{'':<6}{res_reward['calmar']:.2f}", flush=True)
    print(f"==========================================================================================================\n", flush=True)

    summary = {
        'static_frozen_oos': res_static,
        'regime_gated_oos': res_gated,
        'three_factor_reward_plastic_oos': res_reward
    }
    os.makedirs("runs", exist_ok=True)
    with open("runs/quant_reward_modulated_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    print(f"✓ 三因子强化塑性消融审计工件已保存至 runs/quant_reward_modulated_summary.json\n", flush=True)
