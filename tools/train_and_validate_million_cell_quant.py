...
class SixOrganMillionCellBrain(nn.Module):
    """
    六器官模块化百万细胞 (1,000,000 Cells) GPU 张量大脑
    """
    def __init__(self, num_assets=18, num_cells=1000000, num_synapses=2000000, device='cuda'):
        ...
        src_imm  = torch.cat([
            torch.randint(0, 10000, (num_synapses // 10,), dtype=torch.int32, device=device),
            torch.randint(500000, 700000, (num_synapses // 10,), dtype=torch.int32, device=device)
        ])
    ...
    def forward_all_assets(self, batch_features):
        ...
        pos_signal = exec_slice[:, 0:125000].mean(dim=1) * 50.0
        neg_signal = exec_slice[:, 125000:250000].mean(dim=1) * 50.0
        
        immune_slice = self.state[:N, 950000:1000000].float()
        immune_signal = (immune_slice.abs().mean(dim=1) > 1.35) # 仅在极端剧烈冲击下触发
...
def evaluate_brain(brain, data, timeline, d_start, d_end, fee_multiplier=1.0, slippage_multiplier=1.0, force_terminal_liquidation=True):
    ...
    for t, cur_date in enumerate(sub_timeline):
        ...
        if not is_last_day and not trading_halted:
            ...
            for i, sym in enumerate(asset_symbols):
                ...
                target_contracts = min(8, int((total_equity * 0.02) / margin_per_contract)) if margin_per_contract > 0 else 0
                ...
                if imm_val:
                    if cur_qty != 0:
                        immune_triggers += 1
                    target_orders[sym] = 0
                elif target_contracts > 0 and diff > 0.005:
    ...
    return {
        ...
        'win_rate': win_rate,
        'immune_triggers': immune_triggers,
...
def main():
    ...
    print(f"\n========================================================================================================")
    print(f" 🧬 鲲 1,000,000 细胞 GPU 六器官大脑：真实样本内多目标进化训练与样本外盲测大考 🧬")
    print(f"========================================================================================================")
    print(f"• 硬件加速: {torch.cuda.get_device_name(0) if device=='cuda' else 'CPU'} (CUDA Float16)")
    print(f"• 架构形态: 六大器官模块化图谱 (100万细胞, 200万突触, 18 品种独立状态 state[18, 1M])")
    print(f"• 训练制度: 仅在 2005~2015 样本内注入红皇后对抗压力 (1.5x 费率) 进行真实多目标遗传演化")
    print(f"• 盲测制度: 2016~2026 样本外 10.7 年绝对冻结单次只读评估与未训练随机 Reservoir 同台消融")
    print(f"========================================================================================================\n")
    ...
    torch.manual_seed(SEED)
    baseline_random_brain = SixOrganMillionCellBrain(num_assets=num_assets, device=device)

    print(f"[Step 1] 启动 1,000,000 细胞 GPU 张量大脑样本内真实遗传演化 (代数=4, 种群=3, 注入 1.5x 费率对抗压力)...")
    
    population = []
    for pop_i in range(3):
        torch.manual_seed(SEED + pop_i * 100)
        brain_i = SixOrganMillionCellBrain(num_assets=num_assets, device=device)
    ...
    for gen in range(4):
        ...
        for i, brain_cand in enumerate(population):
            res = evaluate_brain(brain_cand, data, timeline, timeline[0], split_date, fee_multiplier=1.5, slippage_multiplier=1.5)
            fit = res['fitness']
        ...
        print(f"  ↳ [Gen {gen}] 种群冠军适应度 = {gen_best_fit:+.2f} (IS CAGR: {gen_best_res['cagr']:+.2f}%, MaxDD: {gen_best_res['max_drawdown']:.1f}%, 平仓笔数: {gen_best_res['total_trades']})")
        ...
        while len(next_gen) < len(population):
            ...
            child = SixOrganMillionCellBrain(num_assets=num_assets, device=device)
    ...
    print(f"\n✓ 百万细胞 GPU 大脑样本内真实多目标进化完成！最高适应度 = {best_fitness:+.2f}\n")

    os.makedirs("runs", exist_ok=True)
    ckpt_path = "runs/quant_million_brain_evolved_champion.pt"
    torch.save(champion_brain.state_dict(), ckpt_path)
    ckpt_size_mb = os.path.getsize(ckpt_path) / (1024 * 1024)
    print(f"✓ 演化冠军 Checkpoint 已保存至 {ckpt_path} (文件大小: {ckpt_size_mb:.2f} MB)")

    print(f"\n========================================================================================================")
    print(f"  📊 样本外 (2016 ~ 2026, 10.7 年) 100 万细胞演化冠军 vs 随机 Reservoir 同台消融 📊")
    print(f"========================================================================================================")
    print(f"{'模型/网络形态':<28}{'期末实际清盘现金':<22}{'总回报率':<14}{'CAGR':<14}{'最大回撤':<14}{'Calmar':<10}{'Sortino':<10}")
    print(f"{'-'*108}")

    res_champ_oos = evaluate_brain(champion_brain, data, timeline, split_date, timeline[-1], fee_multiplier=1.0, slippage_multiplier=1.0)
    print(f"{'🔥 100万细胞演化冠军大脑':<26}{res_champ_oos['final_realized_cash']:,.2f} 元{'':<6}{res_champ_oos['total_roi']:+.2f}%{'':<6}{res_champ_oos['cagr']:+.2f}%{'':<6}{res_champ_oos['max_drawdown']:.2f}%{'':<6}{res_champ_oos['calmar']:.2f}{'':<6}{res_champ_oos['sortino']:.2f}")

    res_rand_oos = evaluate_brain(baseline_random_brain, data, timeline, split_date, timeline[-1], fee_multiplier=1.0, slippage_multiplier=1.0)
    print(f"{'基线: 100万未演化随机网络':<26}{res_rand_oos['final_realized_cash']:,.2f} 元{'':<6}{res_rand_oos['total_roi']:+.2f}%{'':<6}{res_rand_oos['cagr']:+.2f}%{'':<6}{res_rand_oos['max_drawdown']:.2f}%{'':<6}{res_rand_oos['calmar']:.2f}{'':<6}{res_rand_oos['sortino']:.2f}")
    print(f"========================================================================================================\n")

    print(f"🛡️ 样本外 10.7 年独立免疫熔断触发统计:")
    print(f"  • 演化冠军大脑累计触发独立免疫锁闸: {res_champ_oos['immune_triggers']} 次 (有效阻断极端肥尾风险)")
    print(f"  • 随机未演化网络累计触发免疫锁闸: {res_rand_oos['immune_triggers']} 次\n")

    summary = {
        'seed': SEED,
        'checkpoint': ckpt_path,
        'checkpoint_size_mb': ckpt_size_mb,
        'evolved_champion_oos': res_champ_oos,
        'unevolved_random_oos': res_rand_oos
    }
    json_path = "runs/quant_million_evolved_summary.json"
