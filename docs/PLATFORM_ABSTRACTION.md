...
| `benchmark`（进程内总线）不构建 | `sem_init` 在 macOS 已废弃 | 进程内 MessageBus 基准不可用；`benchmark_tcp`（127.0.0.1 loopback）仍构建。TCP 线格式为 v1 紧凑帧（见 `network_transport.h` / book 第 09 章 §6） |
