...
9. **json_to_xodr 网格 junction + laneLink（2026-08-14 已实现并验证）**：`load_scenario` 做**字符串 road id → 数字索引映射** + 保留 `lanes[]`（id/index/direction/successors），city_grid 转出结构化 `<junction>`（每进入方向 fork 含直/左/右 connection），并给每 connection 从 incoming lane successors 生成 **`<laneLink from to/>`**（xodr lane id：正向=-index、对向=+index）、给 incoming road 设 `<link><successor/predecessor elementType="junction" elementId=jid/>`（用 end_states/start_states 几何判断 fork 在 incoming 的哪一端）。实测：city_grid 全图 **14992 laneLink / 2600 junction**，`test_road_network` 确认 esmini 完整加载 1300 roads；demo 跑 route "main" 子集，NPC 沿 laneLink 真实跨路口（`M3 diag: npc1 crossed 0 -> 1`，road 0 的 successor 是指向 junction 2 而非 road 1，故必走 laneLink）。坑 9 关闭。
10. **M3（NPC 路口按需转向）已实现骨架（2026-08）**：`Entity.turn_intent`（0=直 1=左 2=右），spawn 时 60/20/20 随机分配，`step_npc_vehicle` road_pos 推进分支按 intent 设 `junctionSelectorAngle`，过路口（road_id 变化）后重随机防绕圈。**junctionSelectorAngle 语义以 esmini 代码实测为准（0=直 M_PI/2=左 3M_PI/2=右）**——esminiRMLib.hpp:476 头文件注释写 "pi=straight pi/2=right 3pi/2=left" 是错的：`MoveToConnectingRoad` 里 deltaHeading = (连接 road 行驶航向 − 本车行驶航向) mod 2π，直行时恒为 0，绝不可能等于 π；直行错用 M_PI 会在四向路口随机选到垂直支路。已随坑 9 一起修复，laneLink + angle 语义对齐后 M3 在完整大地图生效。
