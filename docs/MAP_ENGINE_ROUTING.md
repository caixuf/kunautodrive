...
9. **json_to_xodr 网格 junction + laneLink（2026-08 根因定位，待实现）**：`load_scenario` 已做**字符串 road id → 数字索引映射**，city_grid 可转出结构化 `<junction>`（每进入方向 fork 含直/左/右 connection），`xodr_compat_check` 静态检查 PASS（1300 roads / 2600 junctions / 5200 lanes，0.24s），`test_road_network` 确认 **esmini 能完整加载 1300 roads 且 frenet↔world roundtrip 全过**。**但 NPC 无法跨路口**：esmini 在 junction 处选路调 `Junction::GetNumberOfRoadConnections(road_id, lane_id)`（RoadManager.cpp:5772），依赖 connection 的 **`<laneLink>`** + incoming road 的 `<link>` 指向 junction——当前 xodr 两者都缺 → 返回 0 → `RM_PositionMoveForward` 失败 → NPC 在 road 段末端 recycle/停住（road 0 上循环）。根因：city_grid 拆段模型"每交叉口每进入方向一个 fork"导致一个 road 段被多个 junction 引用（两端各一），与 OpenDRIVE"一个 road 一个 junction 属性 + link 的 predecessor/successor 各一"语义不匹配。**修复方案（json_to_xodr）**：① load_scenario 保留 `lanes[]`（id/index/direction/successors，当前已压缩成 count 需改回）；② fork 处理时给 incoming road 设 `<link><successor elementType="junction" elementId=jid/>`（用 end_states 几何判断该 fork 在 incoming 的末端）；③ 为每个 connection 从 incoming lane successors 生成 `<laneLink from to/>`（xodr lane id：正向=-index、对向=+index）。工程量中等偏大，需 esmini 运行验证。
10. **M3（NPC 路口按需转向）已实现骨架（2026-08）**：`Entity.turn_intent`（0=直 1=左 2=右），spawn 时 60/20/20 随机分配，`step_npc_vehicle` road_pos 推进分支按 intent 设 `junctionSelectorAngle`（M_PI=直 M_PI/2=右 3M_PI/2=左，esmini 官方语义见 esminiRMLib.hpp:476），过路口（road_id 变化）后重随机防绕圈。**受坑 9 阻塞**：esmini junction 无 laneLink 时 NPC 到不了路口，M3 无法在完整大地图生效（有 junction 的场景均如此）。laneLink 修复后自动生效。
