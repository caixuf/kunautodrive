...
保留原车漆、车灯、轮毂和车身结构。SU7 原始资产的 `Light/LightGlass` 节点会直接复用真实灯罩材质，分别承接近光灯和
刹车灯；只有原资产没有左右语义的转向灯才补四角 overlay，避免替换真实灯具。
`lights` 位掩码和 `brake` 字段驱动亮灭，转向灯按 1.5 Hz 闪烁。真实灯罩保留原始 emissive 纹理，
但使用非色调映射的独立强度（近光 8、刹车 6）和双面材质，确保在白天 HDRI/ACES 场景中仍清晰可见。
