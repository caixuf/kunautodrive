...
* **分层不是配额。** 禁止将「快轨不得超过 $N$ 个细胞」写成生命定律——那只是换了一层泥巴模具。器官边界与时间尺度分层必须由力学相分离与硬件时延选择压自发形成（见 §3.4–§3.5）。`SkeletonLock` / `TypeWhitelist` / `max_cells` 仅是消融实验开关，不是宇宙公理。
...
### 3.4 隔室边界的力学起源：差示粘附，而不是人工限额

分层与器官化的问题不是「我们该不该给底层加限制」，而是：**边界作为物理对象从哪里长出来。** 本系统的第一性原理是力学，不是配额表，也不是预先写好的 Turing 形态素场。

将突触弹簧刚度视为有效粘附功（Steinberg 差示粘附假说的数字对应）：

$$W_{ij} = |w_{ij}|, \qquad \varepsilon_{\mathcal{A}} = \langle |w_{ij}| \rangle_{i,j \in \mathcal{A}}, \qquad \gamma_{\mathcal{AB}} = \frac{\varepsilon_{\mathcal{A}} + \varepsilon_{\mathcal{B}}}{2} - \varepsilon_{\mathcal{AB}}$$

其中 $\mathcal{A},\mathcal{B}$ 为候选细胞团。当界面张力 $\gamma_{\mathcal{AB}} > 0$ 时，两团宁可各自内聚也不互相混溶——**边界就是粘附能的等值面**，也是兰纳-琼斯力与突触张力达到法向力平衡的零通量面：

$$\hat{\mathbf{n}} \cdot \left( \mathbf{F}^{(\mathcal{A})} - \mathbf{F}^{(\mathcal{B})} \right) = \nabla_{\Gamma} \gamma_{\mathcal{AB}}$$

几何上对应三条已经在运行的力：
1. **短程泡利斥力**（$r^{-13}$）：禁止两个算子占据同一空间点，阻止「一团浆糊」；
2. **中程范德华吸引 + 突触弹簧**：$k_{\mathrm{spring}} \propto |w|$ 把强耦合细胞拉成微柱，把弱耦合细胞留在柱间液；
3. **力敏转导生长**：综合应变 $\sigma_i = 0.05\|\mathbf{F}_i\| + 2\,\mathrm{strain}^{\mathrm{info}}_i$ 在高应力处有丝分裂，子细胞沿发育轴 $+z$ 拱起（沟回）。褶皱增加受体面积，不靠把平面直径撑到 DRAM。

因此：**底层反射柱不需要白名单、不需要锁感受器、不需要 `max_cells_fast=256`。** 无效原语付代谢税且不卸载 $\mathrm{strain}^{\mathrm{info}}$，会被凋亡剪掉；有害原语在环境里把车撞了、把仓打穿，个体直接死。底层保持干净，是选择压的结果，不是 if 语句的结果。

> 诚实边界：当前代码**没有**扩散形态素（Turing 反应-扩散）作为化学身份场。细胞类型在有丝分裂时离散抽样，身份靠力学隔室冻结，而不是靠浓度阈值。Turing 可以作为第二机制补「化学标签」，但形状与隔室墙的第一机制是力学。二者在真胚里同时存在；本宇宙目前只实现了力学这一半。

### 3.5 时间尺度分层是谱间隙，不是双轨配额

把有机体在工作点线性化，得到细胞动力学雅可比 $J$。其特征模自然分裂：
* **快局部模**：支撑在高刚度微柱上的局域特征向量——短程、高 $|w|$、图距离小，编译后整柱落入 L1，本征周期落到纳秒–微秒，表现为反射弧；
* **慢全域模**：支撑在弱长程突触上的弥散特征向量——跨隔室、低 $|w|$，本征周期落到毫秒–秒，表现为规划与反事实推演。

快/慢不是我们把网络切成两份名额，而是 **$J$ 的谱间隙 + 缓存层级把谱间隙映射成物理时延。** 谁把百万细胞塞进反射柱，单步就掉出 L1 掉进 DRAM（表 2：1,000,000 细胞 $25.65\,\mathrm{ms}$），在 $20\,\mathrm{kHz}$ 撮合或 $20\,\mathrm{Hz}$ 底盘环里来不及活。硬件时延本身就是选择压。代谢税 $\mathrm{Drain}_{\mathrm{metabolic}}$ 只是体积能，不是天花板：赚得起的皮层可以极大，只是它进不了快特征模。

发育轴 $z$ 在投影 $\Phi$ 中单调，保证图无环——这相当于本宇宙的时间箭头，属于构成性几何，不是细胞配额。$xy$ 平面上的器官边界由 §3.4 的粘附张力决定，$z$ 上的计算深度由沟回拱起决定。分层 = 空间隔室 × 谱间隙，全程没有「底层必须遵守的乱七八糟限制」。
...
为同时兼顾 **24.1 ns 的极限反射时延** 与 **复杂长程因果规划**，编译后的有机体在运行时呈现双时间尺度——这是 §3.5 谱间隙的表现，不是给快轨分配细胞名额：
1. **快轨（ASIL-D 预编译反射弧）**：高刚度微柱被扁平数组映入 L1，由 `Act_ImmuneLock` 门控驱动，以 $24.1\text{ ns}$ 耗时无条件保障物理底盘与盘口熔断不变式；
2. **慢轨（认知前瞻推演）**：慢全域模在 $20\text{Hz}$ 规划周期内展开 $K=16$ 步因果反事实推演：
