...
>
> 本章将深入剖析 FlowEngine 如何仅凭 C11 原生语言特性，优雅且严谨地实现**封装（Encapsulation）、单继承（Single Inheritance）、多态虚表（vtable Dispatch）与运行时生命周期管理**。
...
FlowEngine 采用的策略是：**“C11 微内核抽象底座 + C++20 协程/规控外壳”**。通过在 C 语言中模拟结构清晰的 OOP 范式，既保留了极高的抽象表达力，又拥有以下核心优势：

```
┌─────────────────────────────────────────────────────────────┐
│                   FlowEngine C-OOP 核心价值                  │
├───────────────────┬─────────────────────────────────────────┤
