...
第二个坑和消息大小有关。单条消息的上限是 `MSG_BUS_MAX_DATA_SIZE`，也就是 64 KB。1080P
的 RGB 原图差不多 6 MB，根本放不下。跨进程通道复用同一份 `Message`（见第 07 章），单条
负载同样不能超过 64 KB，也没有“只传一块共享内存 Handle”的借用接口；比 64 KB 更大的
