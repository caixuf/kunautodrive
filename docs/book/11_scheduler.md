...
>
> 传统的独立线程轮询会导致严重的线程上下文切换开销与无序竞争。FlowEngine 设计了 **Choreo 混合调度器（Hybrid Scheduler）**：它结合了经典 FIFO 优先级队列与有向无环图依赖调度，支持 **CPU 核心亲和性绑定（CPU Affinity）、频率限制（RateControl）、资源配额（ResourceQuota）与微秒级延迟追踪（LatencyTracker）**。
...
在 Linux RT 实时内核中，为了避免跨 CPU 核心缓存失效（L1/L2 Cache Miss）与线程抢占抖动，FlowEngine 支持将关键任务硬绑定到指定的 CPU 隔离核心（通过 `isolcpus` 内核启动参数保留的核心）：
...
FlowEngine 的调度器在底层维护一个高效的 Worker 线程池，多个异步 Task/Coroutine 被多路复用调度到预分配的线程池中执行：
